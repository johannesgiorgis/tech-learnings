...
func CLI(args []string) int {
	...
	if err = app.run(); err != nil {
		fmt.Fprintf(os.Stderr, "Runtime erro: %v\n", err)
		return 1
	}
	return 0
}
...
func (app *appEnv) fromArgs(args []string) error {
	...
	f1 := flag.NewFlagSet("xkcd-grab", flag.ContinueOnError)
	f1.IntVar(
		&app.comicNo, "n", LatestComic, "Comic number to fetch (default latest)",
	)
	f1.DurationVar(&app.hc.Timeout, "t", 40*time.Second, "Client timeout")
	f1.BoolVar(
		&app.saveImage, "s", false, "Save image to current directory",
	)
	outputType := f1.String(
		"o", "text", "Print output in format: text/json",
	)
	if err := f1.Parse(args); err != nil {
		return err
	}
